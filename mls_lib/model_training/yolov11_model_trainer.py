...
class YOLOv11Trainer(Task):
    ...
    def execute(self):
        result = self.mlslib_model.train(
            data_path=self.data_path, epochs=self.epochs, imgsz=self.imgsz
        )
        
        df = pd.DataFrame([result.items()]) 
        self.train_result.set_data(df)

        print(
            "Model trained, it was saved in path:", self.mlslib_model.trained_model_path
        )
